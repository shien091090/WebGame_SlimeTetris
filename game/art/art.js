 ...
 * 沿用 v15 的色票、形狀語言與版面骨架; v18 只重做規格變動牽涉到的物件
 * (任務種類「雙消」→「用新形狀方塊」、得到什麼 / 削頂的出現時機、倍率不寫數字、「新形狀方塊」用詞、說明頁 8 頁)。
 */
(function () {
  'use strict';
  ...
  //          reward: 'expand'|'newPiece'|'multiplier'|'none', shave, side: 'left'|'right', fullWidthBonus,
  //          announcing, t, credit }
  ...
  function drawTaskProgress(ctx, s) {
    ...
    // v18: 主獎勵寫在前、削頂在後(與事件內演出順序一致); 削頂只寫「削頂」, 倍率不寫數字
    let reward = s.reward || (kind === 'dig' ? 'expand' : 'newPiece');
    if (reward === 'multiplierMax') reward = 'none'; // 舊名相容: 倍率已達上限 = 沒有主獎勵
    const shave = !!(s.shave || (reward === 'expand' && s.fullWidthBonus));
    const iconKey = reward === 'none' && shave ? 'shave' : reward;
    ...
    else if (reward === 'multiplier') main = [['分數倍率提高', PAL.expand]];
    if (main) {
      const tail = [];
  ...
  function eventBanner(ctx, accent, icon, title, sub, t, next) {
    ...
    if (next && next.kind && t >= 0.7) {
      ...
      txt(ctx, title, tx, sub ? b.y + 19 : b.y + b.h / 2, 17, accent, 'left');
  ...
  const PHASE = { expandShave: [0.3, 0.7], rewardShave: [0.4, 0.7], announce: 0.7 };
  ...
  // state: { shape: 'V'|'U'|'X', t (0~1, 1.5 秒), shave (第偶數個多樣化任務 = true), next: {kind, color, required} }  新外型解鎖事件
  ...
  // state: { capped (倍率已達上限), shave (第偶數個多樣化任務 = true), t (0~1, 1.5 秒), next: {kind, color, required} }  倍率獎勵事件
  // v18 四種: 倍率提高 / 倍率提高 + 削頂 / 已達上限只有削頂 / 已達上限無獎勵(只演尾端公告)。畫面上不寫任何倍率數字
  function drawMultiplierEvent(ctx, s) {
    if (!s) return;
    const t = clamp01(s.t);
    ctx.save();
    const multIcon = function (x, y) { txt(ctx, '×', x + 13, y + 13, 30, PAL.expand, 'center'); };
    const shaveIcon = function (x, y) { rewardIcon(ctx, 'shave', null, x, y, 26); };
    const inShave = s.shave && t >= PHASE.rewardShave[0];
    if (!s.capped) {
      eventBanner(ctx, PAL.expand, multIcon, '獎勵: 分數倍率提高', inShave ? SHAVE_SUB : '右側「分數倍率」已更新', t, s.next);
    } else if (s.shave) {
      eventBanner(ctx, PAL.shave, shaveIcon, '獎勵: 削頂', null, t, s.next);
    } else if (s.next && s.next.kind) {
      eventBanner(ctx, PAL.expand, null, '', null, 1, s.next); // 無獎勵: 整段只演尾端公告
    } else {
      eventBanner(ctx, PAL.textDim, null, '沒有獎勵', null, t, null);
    }
    ctx.restore();
  }
  ...
  // ================= 說明頁 =================
  // v18: 每頁文字照 guide.md 逐字(標題 + 一句); 示意圖裡的遊戲物件一律呼叫遊戲內畫家(P3)
  const GUIDE = [
    { title: '同色連 4 顆就消掉', lines: ['同色上下左右連成 4 顆, 整團消失。'] },
    { title: '消掉的地方會留下洞', lines: ['消掉的地方變成空洞, 上面的史萊姆不會掉下來。'] },
    { title: '任務: 消指定顏色, 盤面變寬', lines: ['這一塊要拿去湊任務的顏色還是別的團, 由你決定。'] },
    { title: '重力球: 能當任何顏色', lines: ['重力球被消掉時, 和它相連的懸空史萊姆會整塊往下掉, 直到碰到東西。'] },
    { title: '開滿之後: 新任務換新形狀方塊', lines: ['盤面長滿後, 任務會一個接一個換成不同的消法。'] },
    { title: '結束與目標: 挑戰最高分', lines: ['下一塊出不來, 或放下的方塊整塊卡在頂線上, 就結束。'] },
    { title: '操作: 方塊', lines: ['按住左右鍵可以連續移動。'] },
    { title: '操作: 暫停、說明、放棄、重開', lines: ['放棄要在遊戲或暫停畫面按住 R 一秒。'] },
  ];
  ...
  FIG[2] = function (ctx) {
    ...
    caption(ctx, '消掉的位置變成空洞', 695, 394);
    caption(ctx, '上面的史萊姆停在原處, 懸在洞上', 695, 420);
    caption(ctx, '青色框 = 懸空的史萊姆(第 4 頁會用到)', 695, 450, PAL.gravity, 'center', 13);
  };
  ...
  FIG[3] = function (ctx) {
    ...
    caption(ctx, '面板寫的那一側(左)', 626, 432, PAL.expand, 'left');
    caption(ctx, '多出一欄空欄', 626, 458, PAL.expand, 'left');
    caption(ctx, '接著換下一個任務', 626, 488, PAL.textDim, 'left', 13);
  };
  ...
  FIG[4] = function (ctx) {
    ...
    // 圖說
    const y0 = 418;
    caption(ctx, '3 顆紅 + 重力球 = 4 顆', L[0] + 91, y0, PAL.text, 'center', 14);
    caption(ctx, '球補上了第 4 顆', L[0] + 91, y0 + 22, PAL.textDim, 'center', 13);
    caption(ctx, '這團消掉了', L[1] + 91, y0, PAL.text, 'center', 14);
    caption(ctx, '原本連著它的那塊: 現在懸空', L[1] + 91, y0 + 22, PAL.gravity, 'center', 13);
    caption(ctx, '另一塊不相連, 也是懸空', L[1] + 91, y0 + 42, PAL.gravity, 'center', 13);
    caption(ctx, '相連的那塊整塊往下掉, 形狀不變', L[2] + 91, y0, PAL.text, 'center', 14);
    caption(ctx, '碰到下面的史萊姆就停, 沒有掉到底', L[2] + 91, y0 + 22, PAL.gravity, 'center', 13);
    caption(ctx, '不相連的那塊原地不動', L[2] + 91, y0 + 42, PAL.textDim, 'center', 13);
    caption(ctx, '青色框 = 懸空的一整塊; 只有和被消那團相連的才會掉。', 480, 512, PAL.text, 'center', 15);
  };
  ...
  FIG[5] = function (ctx) {
    ...
    const k = 0.62;
    placeAt(ctx, BOX.task, 312, 180, k, function () {
      drawTaskProgress(ctx, { kind: 'big', remaining: 1, required: 1, reward: 'newPiece' });
    });
    caption(ctx, '面板上一次只有一個任務', 405, 346, PAL.text, 'center', 13);
    caption(ctx, '做到', 540, 235, PAL.expand, 'center', 14);
    arrow(ctx, 512, 256, 568, 256, PAL.expand, 3);
    placeAt(ctx, BOX.task, 580, 180, k, function () {
      drawTaskProgress(ctx, { kind: 'big', remaining: 0, required: 1, reward: 'newPiece' });
    });
    caption(ctx, '面板顯示已達成', 673, 346, PAL.text, 'center', 13);
    rewardIcon(ctx, 'newPiece', null, 584, 370, 30);
    parts(ctx, [['拿到面板上寫的「', PAL.text], [NEW_PIECE_WORD, PAL.expand], ['」', PAL.text]], 624, 385, 16, 300);
    caption(ctx, '開滿之後還有任務, 做到就拿到面板上寫的東西。', 480, 480, PAL.text, 'center', 15);
  };
  ...
  FIG[6] = function (ctx) {
    ...
    const k = 0.56;
    ctx.save();
    ctx.translate(210 - 350 * k, 150 - 110 * k);
    ctx.scale(k, k);
    ...
    ctx.restore();
    caption(ctx, '堆到頂, 下一塊出不來', 283, 448, PAL.text, 'center', 15);
    placeAt(ctx, { x: 0, y: 0 }, 470, 148, 0.76, function () {
    ...
    caption(ctx, '分數與最佳紀錄', 614, 448, PAL.text, 'center', 15);
    caption(ctx, '挑戰最高分', 614, 474, PAL.textDim, 'center', 13);
  };
